...
//#include "GameModes/PPExperienceManager.h"
...
#include "Private/AssetTypeActions_PPContextEffectsLibrary.h"
...
DEFINE_LOG_CATEGORY(LogPPEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<UPPDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UPPDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FPPEditorModule
 */
class FPPEditorModule : public FDefaultGameModuleImpl
{
	typedef FPPEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FPPEditorModule::ModulesChangedCallback);
		...
		AssetTools.RegisterAssetTypeActions(MakeShareable(new FAssetTypeActions_PPContextEffectsLibrary));
	}

	void OnBeginPIE(bool bIsSimulating)
	{
		//UPPExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<UPPExperienceManager>();
...
IMPLEMENT_MODULE(FPPEditorModule, PPEditor);
