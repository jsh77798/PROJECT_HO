...
#include "PPWeaponStateComponent.generated.h"
...
struct FPPScreenSpaceHitLocation
{
...
struct FPPServerSideHitMarkerBatch
{
	FPPServerSideHitMarkerBatch() { }

	FPPServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FPPScreenSpaceHitLocation> Markers;
...
UCLASS()
class UPPWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UPPWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FPPScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FPPScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FPPServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
