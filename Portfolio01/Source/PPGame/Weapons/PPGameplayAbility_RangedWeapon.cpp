...
#include "PPGameplayAbility_RangedWeapon.h"

#include "Weapons/PPRangedWeaponInstance.h"
#include "Physics/PPCollisionChannels.h"
#include "LogChannels.h"
//#include "AIController.h"
//#include "Messages/PPVerbMessage.h"
#include "NativeGameplayTags.h"
#include "GameFramework/GameplayMessageSubsystem.h"
#include "Weapons/PPWeaponStateComponent.h"
//#include "Teams/PPTeamSubsystem.h"
#include "AbilitySystemComponent.h"
#include "GameFramework/PlayerController.h"
#include "AbilitySystem/PPGameplayEffectContext.h"
#include "AbilitySystem/PPGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PPGameplayAbility_RangedWeapon)

namespace PPConsoleVariables
{
	static float DrawBulletTracesDuration = 0.0f;
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("PP.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("PP.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("PP.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UPPGameplayAbility_RangedWeapon::UPPGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UPPRangedWeaponInstance* UPPGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UPPRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UPPGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogPPAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UPPRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UPPGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UPPGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UPPGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return PP_TraceChannel_Weapon;
}

FHitResult UPPGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UPPGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UPPGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EPPAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UPPWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UPPWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EPPAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EPPAbilityTargetingSource::CameraTowardsFocus) || (Source == EPPAbilityTargetingSource::PawnTowardsFocus) || (Source == EPPAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EPPAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EPPAbilityTargetingSource::WeaponForward) || (Source == EPPAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EPPAbilityTargetingSource::PawnTowardsFocus) || (Source == EPPAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UPPGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (PPConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, PPConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UPPGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UPPRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EPPAbilityTargetingSource::CameraTowardsFocus);
		...
		if (PPConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, PPConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UPPGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UPPRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (PPConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, PPConsoleVariables::DrawBulletHitRadius, FColor::Red, false, PPConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UPPGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UPPGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UPPGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UPPWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPPWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogPPAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UPPGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UPPWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPPWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FPPGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FPPGameplayAbilityTargetData_SingleTargetHit();
