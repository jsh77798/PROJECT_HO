...
#include "PPRangedWeaponInstance.h"

#include "NativeGameplayTags.h"
#include "GameFramework/Pawn.h"
#include "GameFramework/CharacterMovementComponent.h"
#include "Camera/PPCameraComponent.h"
#include "Physics/PPPhysicalMaterialWithTags.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PPRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_PP_Weapon_SteadyAimingCamera, "PP.Weapon.SteadyAimingCamera");

UPPRangedWeaponInstance::UPPRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UPPRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UPPRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UPPRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UPPRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UPPRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UPPRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UPPRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UPPRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UPPRangedWeaponInstance::AddSpread()
{
...
float UPPRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UPPRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	float CombinedMultiplier = 1.0f;
	if (const UPPPhysicalMaterialWithTags* PhysMatWithTags = Cast<const UPPPhysicalMaterialWithTags>(PhysicalMaterial))
	{
...
bool UPPRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UPPRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UPPCameraComponent* CameraComponent = UPPCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_PP_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
