		...
		if (HitComponent)
		{
			...
			if (BoneName == "Head") // 헤드샷( +75% )
			{
				if (HitCharacter)
				{
					...
					Damage += ExtraDamage;
					HitCharacter->TakeDamage(Damage, PointDamageEvent, GetInstigatorController(), this);
					HitCharacter->TakeDamageBone(true, BoneName, PointDamageEvent, GetInstigatorController(), this);
				}
			}
...
void APortfolio_Tile::OtherDamage(FHitResult HitResult_Other)
{
	...
	FPointDamageEvent PointDamageEvent;
	PointDamageEvent.HitInfo = HitResult_Other;
	...
	if (BoneName_Other == "Head") // 헤드샷( +75% )
	{
		if (HitCharacter_Other)
		{
			float ExtraDamage = Damage * 0.75;
			Damage += ExtraDamage;
			HitCharacter_Other->TakeDamage(Damage, PointDamageEvent, GetInstigatorController(), this);
		}
	}
