...
#include "PPDeveloperSettings.generated.h"

//class UPPExperienceDefinition;
...
class UPPDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UPPDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PP, meta=(AllowedTypes="PPExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(BlueprintReadOnly, config, Category=PPBots)
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PPBots, meta=(EditCondition=bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PPBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PP)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in PP is to only play force feedback if the most recent input device was a gamepad.
	*/
	//UPROPERTY(config, EditAnywhere, Category = PP, meta = (ConsoleVariable = "PPPC.ShouldAlwaysPlayForceFeedback"))
	...
	UPROPERTY(config, EditAnywhere, Category=PP)
	TArray<FPPCheatToRun> CheatsToRun;
...
public:
	...
	PPGAME_API void OnPlayInEditorStarted() const;
