...
#include "PPCameraMode_ThirdPerson.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "PPCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PPCameraMode_ThirdPerson)

namespace PPCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UPPCameraMode_ThirdPerson::UPPCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FPPPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UPPCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UPPCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UPPCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UPPCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IPPCameraAssistInterface* TargetControllerAssist = Cast<IPPCameraAssistInterface>(TargetController);

	IPPCameraAssistInterface* TargetActorAssist = Cast<IPPCameraAssistInterface>(TargetActor);
	...
	IPPCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IPPCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IPPCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IPPCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UPPCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IPPCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FPPPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(PPCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UPPCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UPPCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
