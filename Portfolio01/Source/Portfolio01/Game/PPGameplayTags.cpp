...
FPPGameplayTags FPPGameplayTags::GameplayTags;

void FPPGameplayTags::InitializeNativeTags()
{
...
void FPPGameplayTags::AddAllTags(UGameplayTagsManager& Manager)
{
...
void FPPGameplayTags::AddTag(FGameplayTag& OutTag, const ANSICHAR* TagName, const ANSICHAR* TagComment)
{
	OutTag = UGameplayTagsManager::Get().AddNativeGameplayTag(FName(TagName), FString(TEXT("(Native) ")) + FString(TagComment));
}

void FPPGameplayTags::AddMovementModeTag(FGameplayTag& OutTag, const ANSICHAR* TagName, uint8 MovementMode)
{
	AddTag(OutTag, TagName, "Character movement mode tag.");
	GameplayTags.MovementModeTagMap.Add(MovementMode, OutTag);
}

void FPPGameplayTags::AddCustomMovementModeTag(FGameplayTag& OutTag, const ANSICHAR* TagName, uint8 CustomMovementMode)
{
...
FGameplayTag FPPGameplayTags::FindTagByString(FString TagString, bool bMatchPartialString)
{
	...
	if (!Tag.IsValid() && bMatchPartialString)
	{
		...
		for (const FGameplayTag TestTag : AllTags)
		{
			...
			{
				UE_LOG(LogPP, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
				Tag = TestTag;
				break;
			}
		}
