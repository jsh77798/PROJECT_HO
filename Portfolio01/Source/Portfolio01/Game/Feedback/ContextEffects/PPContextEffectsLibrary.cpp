...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PPContextEffectsLibrary)


void UPPContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UPPContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UPPContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UPPContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FPPContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UPPActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FPPContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UPPActiveContextEffects* NewActiveContextEffects = NewObject<UPPActiveContextEffects>(this);
	...
	this->PPContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UPPContextEffectsLibrary::PPContextEffectLibraryLoadingComplete(
	TArray<UPPActiveContextEffects*> PPActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(PPActiveContextEffects);
}
