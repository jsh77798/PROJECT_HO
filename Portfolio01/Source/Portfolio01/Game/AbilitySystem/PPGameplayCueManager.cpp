...
#include "PPGameplayCueManager.h"
...
#include "Game/System/PPAssetManager.h"
#include "Async/Async.h"
#include "Algo/Transform.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PPGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EPPEditorLoadMode
{
...
namespace PPGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("PP.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via PPGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UPPGameplayCueManager::DumpGameplayCues));

	static EPPEditorLoadMode LoadMode = EPPEditorLoadMode::LoadUpfront;
}
...
UPPGameplayCueManager::UPPGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UPPGameplayCueManager* UPPGameplayCueManager::Get()
{
	return Cast<UPPGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UPPGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UPPGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogPP, Warning, TEXT("UPPGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UPPGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (PPGameplayCueManagerCvars::LoadMode)
	{
	case EPPEditorLoadMode::LoadUpfront:
		return true;
	case EPPEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EPPEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UPPGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UPPGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UPPGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UPPGameplayCueManager* GCM = Cast<UPPGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogPP, Error, TEXT("DumpGameplayCues failed. No UPPGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogPP, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogPP, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogPP, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogPP, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogPP, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogPP, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogPP, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogPP, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogPP, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogPP, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogPP, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogPP, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UPPGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UPPGameplayCueManager* StrongThis = Get())
					{
...
void UPPGameplayCueManager::HandlePostGarbageCollect()
{
...
void UPPGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogPP, Warning, TEXT("UPPGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UPPGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (PPGameplayCueManagerCvars::LoadMode)
	{
	case EPPEditorLoadMode::LoadUpfront:
...
void UPPGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UPPGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UPPGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UPPGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UPPGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UPPGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
